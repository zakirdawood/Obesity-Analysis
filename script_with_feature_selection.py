...
data = data[["Age", "Weight", "family_history_with_overweight", "CAEC"]]
...
plt.title('Decision Tree Accuracies at Different Depths With Feature Selection')
plt.show()

# plot the decision tree at depth 8
final_clf = tree.DecisionTreeClassifier(max_depth=8, random_state=74)
...
plt.title("Decision Tree With Feature Selection At Depth of 8")
plt.show()

# print accuracy at depth of 6
print("Training Accuracy at Decision Tree Depth of 8: " + str(round(training_accuracies[7], 4)))
print("Test Accuracy at Decision Tree Depth of 8: " + str(round(testing_accuracies[7], 4)))
...
plt.title('PCA With Feature Selection')
plt.grid(True)
plt.show()
...
plt.title("Complete Link Hierarchical Clustering With Feature Selection")
plt.tight_layout()
plt.show()
...
plt.title("Silhouette Score vs. Number of Clusters With Feature Selection")
plt.grid(True)
plt.show()

# show last 4 clusters on original pca graph
clusters = fcluster(complete_link, t=4, criterion='maxclust')
plt.scatter(pca_data[:, 0], pca_data[:, 1], c=clusters)
plt.xlabel('PCA Component 1')
plt.ylabel('PCA Component 2')
plt.title('Complete Linkage Clusters on PCA Data With Feature Selection')
plt.grid(True)
plt.show()

# silhoutte score
silhoutte = silhouette_score(pca_data, clusters)
print("Silhouette Score (Complete-Link with 4 clusters): " + str(silhoutte))
...
plt.title('K-means Inertia vs. Number of Clusters With Feature Selection')
plt.xlabel('Clusters')
plt.ylabel('Inertia')
plt.show()

# kmeans with elbow value of k=4 
kmeans = KMeans(n_clusters=4, random_state=74)
...
plt.ylabel('Score')
plt.title('KNN Metrics Score vs. Number of Neighbors With Feature Selection')
plt.legend()
plt.grid(True)
plt.show()

# train and predict knn on k value with best metrics (k=3)
...
disp = ConfusionMatrixDisplay(confusion_matrix=cm, display_labels=label_encoder.classes_)
disp.plot(xticks_rotation='vertical')
plt.title(f'KNN Confusion Matrix With Feature Selection')
plt.show()
